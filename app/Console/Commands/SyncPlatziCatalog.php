...
class SyncPlatziCatalog extends Command
{
    ...
    protected function syncProducts(string $url, int $limit, bool $download, bool $dryRun, int $max): int
    {
        ...
        while (true) {
            ...
            foreach ($items as $p) {
                // API shape esperado:
                // { id, title, price, description, images: [..], category: { id, name, image } }
                ...
                // Resolver categoría local por external_id
                ...
                // Stock: la API no trae stock; definimos heurística
                $stock = 50;
            ...
            $offset += $limit;
            // Pequeña pausa para ser buen ciudadano
            usleep(200000); // 200ms
        }
    ...
    {
        // Borramos y recreamos (simple y consistente)
